<div id="header" class="makent-header {{ (!isset($exception)) ? (Route::current()->uri() == '/' ? 'shift-with-hiw' : '') : '' }}">
  <header class="header--sm show-sm" aria-hidden="true" role="banner">
    <a href="javascript:void(0);" style="background-image: url('http://honestrooms.herokuapp.com/images/logos/logo.png'); background-size: 20%;" href="{{ url('/') }}"  aria-label="Homepage" data-prevent-default="" class="header-logo link-reset burger--sm">
...
<header class="regular-header clearfix hide-sm" id="old-header" role="banner" style="background: none !important;">
  ...
  <a aria-label="Homepage" style="background-image: url('http://honestrooms.herokuapp.com/images/logos/logo.png'); position: absolute;height: 100px !important;width: 300px !important;margin-top: -30px;" href="{{ url('/') }}" class="header-belo header-logo pull-left {{ (!isset($exception)) ? (Route::current()->uri() == '/' ? 'home-logo' : '') : '' }}" >
    <span class="screen-reader-only">
      {{ $site_name }}
    </span>
  </a>
